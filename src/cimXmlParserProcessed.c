...
/*
 * $Id: cimXmlParser.c,v 1.39 2012/12/12 22:09:30 buccella Exp $
 *
...
int sfcXmllex(YYSTYPE * lvalp, ParserControl * parm)
{
   int i, m, rc;
   char *next;

   _SFCB_ENTER(TRACE_XMLPARSING, "sfcXmllex");
	...
	/* skip comment section */
         ...
         for (i = 0, m = sizeof(tags)/sizeof(Tags); i < m; i++) {
            if (nextEquals(next, tags[i].tag) == 1) {
               ...
               // fprintf(stderr, " sfcXmllex| processing for: %.10s\n", next);
...
int sfcXmlerror(char *s)
{
   printf("*-* sfcXmlerror: %s\n", s);
   return 5;
   //   exit(5);
}
...
//      printf("--- setjmp caught !!\n");
   ...
   control.reqHdr.rc = sfcXmlparse(&control);
