...
static const char *const yytname[] =
{
  ...
  "havingClause", "$@12", "orderByClause", "$@13", "sortKeyList",
  "sortKey", "OPT_ASC_DESC", "fetchFirstClause", "searchCondition",
  "optCondition", "optLimCondition", "joinCondition", "predicate",
  "limitedPredicate", "colIdentifier", "number", "comparison", "optNot", 0
};
#endif
...
#line 282 "sqlParser.y"
    {
	  ClassDef * cd = (ClassDef*)(yyvsp[(2) - (4)].classDef); 
	  //printf("call abgearbeitet\n");
	  ...
	  assignmentList->ft->append(assignmentList,el);
	  */
	  Call * call = newCall(cd->className,(yyvsp[(4) - (4)].strValue),assignmentList,whereList);
...
#line 336 "sqlParser.y"
    {
	  ...
	  for(i=0,col = (Column*)columnList->ft->getFirst(columnList);col;col = (Column*)columnList->ft->getNext(columnList),i++){
	    if(strcasecmp(col->colName,(yyvsp[(2) - (5)].strValue))==0){
	      found = 1;
	      //printf("::: %s %s\n",$2,$4);
	      el = newExpressionLight((yyvsp[(2) - (5)].strValue),UNDEF,(yyvsp[(4) - (5)].strValue));
	  ...
	  if(found==0){
	    yyerror(semanticError("UNDIFINED PARAMETER:\nColumn \"",(yyvsp[(2) - (5)].strValue),"\" is not part of the Key"));STM->rs->sw->sqlstate = "42P02";YYERROR;
	  }
...
#line 364 "sqlParser.y"
    {
	  ExpressionLight * el;
	  el = newExpressionLight((yyvsp[(2) - (7)].strValue),UNDEF,(yyvsp[(4) - (7)].strValue));
	  el->sqltype = (yyvsp[(6) - (7)].intValue);
	  //printf("::: %s %s\n",$2,$4);
	  assignmentList->ft->append(assignmentList,el);
	}
    break;
...
#line 373 "sqlParser.y"
    {
		...
		assignmentList = newList();
		ClassDef* cd = (ClassDef*)(yyvsp[(2) - (2)].classDef);
...
#line 390 "sqlParser.y"
    {
		//printf("fertig...\n");
		ClassDef* cd = (ClassDef*)(yyvsp[(2) - (7)].classDef);
...
#line 402 "sqlParser.y"
    {
		//printf("assignment\n");
		ExpressionLight * exl = (ExpressionLight*)(yyvsp[(3) - (3)].exl);
		if(exl->name==NULL)
			exl->name = (yyvsp[(1) - (3)].strValue);
		Column* col;
		Column* rcol = NULL;
		//printf("assignment2\n");
		for(col = (Column*)allColumnsList->ft->getFirst(allColumnsList);col;col = (Column*)allColumnsList->ft->getNext(allColumnsList)){
			if(strcasecmp(col->colName,(yyvsp[(1) - (3)].strValue))==0){
				rcol = col;
				//printf("gefunden!!!\n");
			}
		...
		for(exl2 = (ExpressionLight*)assignmentList->ft->getFirst(assignmentList);exl2;exl2 = (ExpressionLight*)assignmentList->ft->getNext(assignmentList)){
			if(strcasecmp(exl->name,exl2->name)==0){
				yyerror(semanticError("DUPLICATE COLUMN:\nCannot change column \'",(yyvsp[(1) - (3)].strValue),"\' more then once in one statement"));STM->rs->sw->sqlstate = "42701";YYERROR;
			}
		...
		if(rcol==NULL){
			...
			yyerror(semanticError("UNDIFINED COLUMN:\nColumn \'",(yyvsp[(1) - (3)].strValue),"\' in assigmnet-clause does not exist"));STM->rs->sw->sqlstate = "42703";YYERROR;
	  	}	
		
		if(exl->name!=NULL&&strcasecmp(rcol->colName,exl->name)!=0){
			
			yyerror(semanticError("ERROR IN ASSIGNMENT:\nOperation can only be applied to same column: \"",(yyvsp[(3) - (3)].exl),"\" "));STM->rs->sw->sqlstate = "22005";YYERROR;
		}
		//printf("assignment4\n");
...
#line 479 "sqlParser.y"
    {(yyval.exl) = newExpressionLight((yyvsp[(1) - (3)].strValue),PLUS,(yyvsp[(3) - (3)].strValue));}
    break;
...
#line 480 "sqlParser.y"
    {(yyval.exl) = newExpressionLight((yyvsp[(1) - (3)].strValue),SUB,(yyvsp[(3) - (3)].strValue));}
    break;
...
#line 481 "sqlParser.y"
    {(yyval.exl) = newExpressionLight((yyvsp[(1) - (3)].strValue),MUL,(yyvsp[(3) - (3)].strValue));}
    break;
...
#line 482 "sqlParser.y"
    {(yyval.exl) = newExpressionLight((yyvsp[(1) - (3)].strValue),DIV,(yyvsp[(3) - (3)].strValue));}
    break;
...
#line 483 "sqlParser.y"
    {(yyval.exl) = newExpressionLight(NULL,CONCAT,(yyvsp[(2) - (2)].strValue));}
    break;
...
#line 484 "sqlParser.y"
    {(yyval.exl) = newExpressionLight(NULL,ASS,(yyvsp[(1) - (1)].strValue));}
    break;
...
#line 485 "sqlParser.y"
    {(yyval.exl) = newExpressionLight(NULL,ASN,(yyvsp[(1) - (1)].strValue));}
    break;
...
#line 489 "sqlParser.y"
    {
		...
		allColumnsList = newList();
		assignmentList = newList();
		ClassDef* cd = (ClassDef*)(yyvsp[(3) - (3)].classDef);
		UtilList* ul = cd->fieldNameList;
		int i;
		for(i=0; i<cd->fieldCount; i++){
			Column * col = (Column*)ul->ft->getNext(ul);
...
#line 504 "sqlParser.y"
    {
		//printf("fertig... \n");
		ClassDef* cd = (ClassDef*)(yyvsp[(3) - (5)].classDef);
		//printf("fertig... %s\n",cd->className);
...
#line 586 "sqlParser.y"
    {
		...
		Column* col;
		for(col = (Column*)allColumnsListIns->ft->getFirst(allColumnsListIns);col;col = (Column*)allColumnsListIns->ft->getNext(allColumnsListIns)){
			if(strcasecmp(col->colName,(yyvsp[(1) - (1)].strValue))==0){
				rcol = col;
			}
			if(rcol!=NULL)
				break;
		...
		if(rcol==NULL){
			yyerror(semanticError("UNDEFINED COLUMN:\nColumn \'",(yyvsp[(1) - (1)].strValue),"\' in colList-clause of INSERT-Statement does not exist"));STM->rs->sw->sqlstate = "42703";YYERROR;
	  	}
	  	ExpressionLight * el;
		for(el = (ExpressionLight*)assignmentList->ft->getFirst(assignmentList);el;el = (ExpressionLight*)assignmentList->ft->getNext(assignmentList)){
			if(strcasecmp(el->name,(yyvsp[(1) - (1)].strValue))==0){
				yyerror(semanticError("DUPLICATED COLUMN:\nColumn \'",(yyvsp[(1) - (1)].strValue),"\' indicated twice"));STM->rs->sw->sqlstate = "42701";YYERROR;
			}
		}
		assignmentList->ft->append(assignmentList,newExpressionLight((yyvsp[(1) - (1)].strValue),UNDEF,NULL));	
	}
    break;
...
#line 745 "sqlParser.y"
    {
		...
		switch (col->colSQLType) {
	  	...
	  	char buf[100];
		int ii = sprintf(buf,"%d",(yyvsp[(1) - (1)].intValue));
...
#line 795 "sqlParser.y"
    {
		...
		switch (col->colSQLType) {
	  	...
	  	char buf[100];
		int ii = sprintf(buf,"%f",(yyvsp[(1) - (1)].doubleValue));
		char *tmp = (char*) calloc(1,ii+1);
		strncpy(tmp,buf,ii);
	  	el->sqltype = col->colSQLType; 
	  	el->value = tmp; 
...
#line 891 "sqlParser.y"
    {
		//printf("suche: %s\n",$3);
		ClassDef * cd = getClassDef(STM->db,(yyvsp[(3) - (6)].strValue));
		if(cd!=NULL){	
			yyerror(semanticError("DUPLICATE TABLE:\nTable \"",(yyvsp[(3) - (6)].strValue),"\" does alrady exist"));STM->rs->sw->sqlstate = "42P07";YYERROR;
		}
		
		STM->addNode(STM,CREATE,newClassDef(0,(yyvsp[(3) - (6)].strValue),eList,0,NULL));
...
#line 901 "sqlParser.y"
    {
		//printf("suche: %s\n",$3);
		ClassDef* cd1 = (ClassDef*)(yyvsp[(5) - (6)].classDef);
		ClassDef * cd = getClassDef(STM->db,(yyvsp[(3) - (6)].strValue));
		if(cd!=NULL){	
			yyerror(semanticError("DUPLICATE TABLE:\nTable \"",(yyvsp[(3) - (6)].strValue),"\" does alrady exist"));STM->rs->sw->sqlstate = "42P07";YYERROR;
		}
		STM->addNode(STM,CREATE,newClassDef(0,(yyvsp[(3) - (6)].strValue),NULL,0,cd1->className));
	  }
    break;
...
#line 917 "sqlParser.y"
    {//columnOptions {
		if(eList==NULL)
			eList=newList();
			//printf("DT: %d %d\n",$2,CMPI_real32);
		eList->ft->append(eList,newColumn(NULL,NULL, (yyvsp[(1) - (3)].strValue), NULL, (yyvsp[(2) - (3)].intValue),(yyvsp[(3) - (3)].intValue)));//CMPI_real32
...
#line 1027 "sqlParser.y"
    {
		STM->addNode(STM,SIGMA,newSelection(obList,(int)(yyvsp[(5) - (5)].intValue)));//$3 statt NULL
		//printf("fetchfirs:  %d\n",(int)$5);
		obList=NULL;	
	}
    break;
...
#line 1040 "sqlParser.y"
    {
		...
		int ret = STM->addNode(STM,SUB,(SubSelect*)(yyvsp[(2) - (2)].subSelect));
...
#line 1124 "sqlParser.y"
    {
		...
		Projection* prj = (Projection*)(yyvsp[(1) - (7)].projection);
		
		Selection* sigma = newSelection(obList,(yyvsp[(7) - (7)].intValue));
		SubSelect* sbs = (SubSelect*)(yyvsp[(2) - (7)].subSelect);
		sbs->sigma = sigma;
		sbs->pi = prj;
...
#line 1147 "sqlParser.y"
    {
		(yyval.projection) = newProjection((yyvsp[(2) - (3)].intValue),(UtilList*)(yyvsp[(3) - (3)].list));
...
#line 1182 "sqlParser.y"
    {
			...
			Column *col = newColumn((yyvsp[(3) - (5)].strValue),(yyvsp[(3) - (5)].strValue),(yyvsp[(5) - (5)].strValue),NULL,UNDEF,UNDEF);
...
#line 1193 "sqlParser.y"
    {
			...
			Column *col = newColumn((yyvsp[(1) - (3)].strValue),(yyvsp[(1) - (3)].strValue),(yyvsp[(3) - (3)].strValue),NULL,UNDEF,UNDEF);
			columnList->ft->append(columnList,col);
			allColumnsList->ft->append(allColumnsList,col); 	
			if((yyvsp[(1) - (3)].strValue)!=NULL)
				aliasList->ft->append(aliasList,(yyvsp[(1) - (3)].strValue));	
		}
    break;
...
#line 1204 "sqlParser.y"
    {
			...
			Column *col = newColumn(NULL,NULL,(yyvsp[(3) - (4)].strValue),(yyvsp[(4) - (4)].strValue),UNDEF,UNDEF);
			columnList->ft->append(columnList,col);
			allColumnsList->ft->append(allColumnsList,col); 	
		}
    break;
...
#line 1213 "sqlParser.y"
    {
			...
			Column *col = newColumn(NULL,NULL,(yyvsp[(1) - (2)].strValue),(yyvsp[(2) - (2)].strValue),UNDEF,UNDEF);
			columnList->ft->append(columnList,col);
			allColumnsList->ft->append(allColumnsList,col); 	
		}
    break;
...
#line 1295 "sqlParser.y"
    {
		...
		tableReferenceList->ft->append(tableReferenceList,(yyvsp[(1) - (1)].subSelect));
	}
    break;
...
#line 1301 "sqlParser.y"
    {
		if(tableReferenceList==NULL)
			tableReferenceList = newList();
		tableReferenceList->ft->append(tableReferenceList,(yyvsp[(3) - (3)].subSelect));	
	}
    break;
...
#line 1309 "sqlParser.y"
    {
		ClassDef* cd = (ClassDef*)(yyvsp[(1) - (2)].classDef);
		Column *colA, *colB;
		UtilList* ul = cd->fieldNameList;
			...
			for(colA = (Column*)columnList->ft->getFirst(columnList); colA; colA = (Column*)columnList->ft->getNext(columnList)){//gefundene Spalten mit classdef vergleichen. 
				...
				for(colB = (Column*)ul->ft->getFirst(ul); colB; colB = (Column*)ul->ft->getNext(ul)){			
					if(strcasecmp(colB->colName,colA->colName)==0){
						...
						if(colA->tableName==NULL || (strcasecmp(colA->tableName,(yyvsp[(2) - (2)].strValue))==0 || strcasecmp(colA->tableName,cd->className)==0)){
...
//								
							...
							for(alias = (char*)aliasList->ft->getFirst(aliasList); alias; alias = (char*)aliasList->ft->getNext(aliasList)){
								if(strcasecmp(alias,(yyvsp[(2) - (2)].strValue))==0)
									aliasList->ft->removeCurrent(aliasList);
								
							}
							if(strcmp(" ",(yyvsp[(2) - (2)].strValue))==0)
								colA->tableAlias = cd->className;
							else
								colA->tableAlias = (yyvsp[(2) - (2)].strValue);
		...
		sbs->aliasName = (char*)calloc(1,strlen((yyvsp[(2) - (2)].strValue))+1);
		strcpy(sbs->aliasName,(yyvsp[(2) - (2)].strValue));
...
#line 1377 "sqlParser.y"
    {
		(yyval.subSelect) = newSubSelectC((yyvsp[(1) - (6)].subSelect),(yyvsp[(4) - (6)].subSelect),(yyvsp[(2) - (6)].intValue));
...
#line 1390 "sqlParser.y"
    {
		//printf("suche: %s\n",$1);
		ClassDef * cd = getClassDef(STM->db,(yyvsp[(1) - (1)].strValue));
		if(cd==NULL){	
			yyerror(semanticError("UNDIFINED TABLE:\nTable \"",(yyvsp[(1) - (1)].strValue),"\" does not exist"));STM->rs->sw->sqlstate = "42P01";YYERROR;
		}
		else{
...
#line 1446 "sqlParser.y"
    {//orderby nur auf columns, die in pi angegeben wurden!!!
		...
		//int found = 0;
		int colnr = (yyvsp[(1) - (2)].intValue);
		char num[4];
		
		if(colnr>columnList->ft->size(columnList)||colnr<1){			
		  sprintf(num,"%d",(yyvsp[(1) - (2)].intValue));
		  yyerror(semanticError("UNDIFINED COLUMN:\nColumn number \"",num,"\" in order-by-clause does not exist in the result relation"));STM->rs->sw->sqlstate = "42703";YYERROR;
			...
			obList->ft->append(obList,newOrder(colnr,t,(yyvsp[(2) - (2)].intValue)));
...
#line 1470 "sqlParser.y"
    {
		...
		int i, found;
		for(i=0,col = (Column*)columnList->ft->getFirst(columnList);col;col = (Column*)columnList->ft->getNext(columnList),i++){
			if(strcasecmp(col->colName,(yyvsp[(1) - (2)].strValue))==0||strcasecmp(col->colAlias,(yyvsp[(1) - (2)].strValue))==0){
				found = 1;
				obList->ft->append(obList,newOrder(i,col->colSQLType,(yyvsp[(2) - (2)].intValue)));
				//printf("key found\n");
				break;
			}
		}
		if(found==0){
			yyerror(semanticError("UNDEFINED COLUMN:\nColumn \"",(yyvsp[(1) - (2)].strValue),"\" in order-by-clause does not exist"));STM->rs->sw->sqlstate = "42703";YYERROR;
		}
	}
    break;
...
#line 1495 "sqlParser.y"
    {
		if((yyvsp[(3) - (5)].intValue)<1){
			...
			(yyval.intValue)=(yyvsp[(3) - (5)].intValue);	
	}
    break;
...
#line 1506 "sqlParser.y"
    {
		Sigma *sigma = (Sigma*) (yyvsp[(2) - (3)].sigma);
		if((yyvsp[(1) - (3)].intValue)==NOT)
			sigma->negate(sigma);
...
#line 1515 "sqlParser.y"
    {//optnot dreht die Einträge, die man aus predicate erhält, um
		Sigma *sigma = (Sigma*) (yyvsp[(3) - (4)].sigma);
		sigma->link = AND;
		if((yyvsp[(2) - (4)].intValue)==NOT)
			sigma->negate(sigma);
		whereList->ft->append(whereList,sigma);	
	}
    break;
...
#line 1532 "sqlParser.y"
    {
		Sigma *sigma = (Sigma*) (yyvsp[(3) - (3)].sigma);
		sigma->link = AND;
		if((yyvsp[(2) - (3)].intValue)==NOT)
			sigma->negate(sigma);
		whereList->ft->append(whereList,sigma);	
	}
    break;
...
#line 1563 "sqlParser.y"
    {//ursprünglich stand hier eine expression. bis auf weiteres zurückgestellt
	  	//printf("join pred\n");
	  	Column* colA = (Column*)(yyvsp[(1) - (3)].column);
		Column* colB = (Column*)(yyvsp[(3) - (3)].column);
		...
		int typeB = colB->colSQLType;
	  	...
	  	(yyval.sigma) = newSigma(colA,(yyvsp[(2) - (3)].intValue),NULL,colB,NULL);//INNER oder FULL????
	}
    break;
...
#line 1576 "sqlParser.y"
    {
		...
		Column* col = (Column*)(yyvsp[(1) - (3)].column);
		...
		//printf("%s %s %s %s %d\n",col->colName,col->colAlias,col->tableName,col->tableAlias,type);
	  	...
	  	(yyval.sigma) = newSigma(col,(yyvsp[(2) - (3)].intValue),(yyvsp[(3) - (3)].strValue),NULL,NULL);
	  	//printf("%s %p %p\n",colnameA,colnameA,$$);
	}
    break;
...
#line 1630 "sqlParser.y"
    {//zwei Einträge ex >a AND ex<b == ex between a and b
		...
		//printf("::: between\n");
	  	Column* col = (Column*)(yyvsp[(1) - (6)].column);
		...
		int ni = 0;
	  	...
	  	int between = ((yyvsp[(2) - (6)].intValue)==NOT)? NOTBETWEEN : BETWEEN;
	  	double a = strtod((yyvsp[(4) - (6)].strValue),(char**)NULL);
	  	double b = strtod((yyvsp[(6) - (6)].strValue),(char**)NULL);
	  	if(a<b)
	  		(yyval.sigma) = newSigma(col,between,(yyvsp[(4) - (6)].strValue),NULL,(yyvsp[(6) - (6)].strValue));
	  	 else
	  		(yyval.sigma) = newSigma(col,between,(yyvsp[(6) - (6)].strValue),NULL,(yyvsp[(4) - (6)].strValue));
...
#line 1658 "sqlParser.y"
    {
		Column* col = (Column*)(yyvsp[(1) - (4)].column);
		
		int not = ((yyvsp[(3) - (4)].intValue)==NOT)? NOTNULL : ISNULL;      
...
#line 1669 "sqlParser.y"
    {
		...
		Column* col;
		for(col = (Column*)allColumnsList->ft->getFirst(allColumnsList);col;col = (Column*)allColumnsList->ft->getNext(allColumnsList)){
			if(strcasecmp(col->colName,(yyvsp[(1) - (1)].strValue))==0||strcasecmp(col->colAlias,(yyvsp[(1) - (1)].strValue))==0){
	  	...
	  	if(rcol==NULL){
			yyerror(semanticError("UNDIFINED COLUMN:\nColumn \'",(yyvsp[(1) - (1)].strValue),"\' in SEARCH-CONDITION does not exist"));STM->rs->sw->sqlstate = "42703";YYERROR;
	  	}	
	  	(yyval.column) = rcol;
	}
    break;
...
#line 1685 "sqlParser.y"
    {
		...
		for(aliasc = (char*)aliasList->ft->getFirst(aliasList);aliasc;aliasc = (char*)aliasList->ft->getNext(aliasList)){
			if(strcasecmp(aliasc,(yyvsp[(1) - (3)].strValue))==0||strcasecmp(aliasc,(yyvsp[(1) - (3)].strValue))==0){
				alias = aliasc;
			}
		...
		for(col = (Column*)allColumnsList->ft->getFirst(allColumnsList);col;col = (Column*)allColumnsList->ft->getNext(allColumnsList)){
			if((strcasecmp(col->colName,(yyvsp[(3) - (3)].strValue))==0||strcasecmp(col->colAlias,(yyvsp[(3) - (3)].strValue))==0)&&(alias!=NULL||strcasecmp(col->tableName,(yyvsp[(1) - (3)].strValue)))){
				rcol = col;
			}
	  	...
	  	if(rcol==NULL){
			yyerror(semanticError("UNDIFINED COLUMN:\nColumn \'",(yyvsp[(3) - (3)].strValue),"\' in SEARCH-CONDITION does not exist"));STM->rs->sw->sqlstate = "42703";YYERROR;
	  	}	
	  	(yyval.column) = rcol;
	}
    break;
...
#line 1714 "sqlParser.y"
    {//printf("::: number1\n");
		char buf[100];
		int i = sprintf(buf,"%d",(yyvsp[(1) - (1)].intValue));
...
#line 1721 "sqlParser.y"
    {
		char buf[100];
		int i = sprintf(buf,"%f",(yyvsp[(1) - (1)].doubleValue));
